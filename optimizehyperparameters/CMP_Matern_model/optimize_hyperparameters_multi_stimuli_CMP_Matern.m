...
%           Matern_order: the order of the Matern kernal (0 or 1)
...
function hyperparameter_final = optimize_hyperparameters_multi_stimuli_CMP_Matern(ysamp,xsamp,dtbin,Matern_order,optimization_method,cordinate_update_iterations_max,min_change_in_hyp,tolerance_lambda)
...
%             options = optimoptions('fmincon','Display','iter','PlotFcn',@optimplotfval,'StepTolerance',1e-2,'OptimalityTolerance',1e-2,'MaxFunctionEvaluations',100);
            ...
            hyperparameter_init = specify_fmincon_par_multi_stimuli_CMP_Matern(J); % Since the domain is constrained in this case, the algorithm converges to the true parameter settings even without a grid search
        ...
        case 'fminunc'
            ...
            hyperparameter_init = specify_fmincon_par_multi_stimuli_Matern(J); % Since the domain is constrained in this case, the algorithm converges to the true parameter settings even without a grid search
...
%             fprintf('\n\n Estimating signal hyper-parameters for stimulus %d \n---------------------------------------------------\n', p);
            ...
            estimate_signal = @(signal_rho_len)derive_ELBO_VI_CMP_Matern(signal_rho_len,noise_rho_len,xsamp,squeeze(ysamp(:,:,j)),dtbin,tolerance_lambda,use_log, Matern_order);  
        ...
        estimate_noise = @(noise_rho_len)derive_ELBO_VI_multi_stimuli_CMP_Matern(signal_rho_len,noise_rho_len,xsamp,ysamp,dtbin,tolerance_lambda,use_log, Matern_order);  
