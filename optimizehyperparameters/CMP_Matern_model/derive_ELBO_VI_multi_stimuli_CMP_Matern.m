...
function [ELBO,mu_all,B_nu_all,lambda_hat_all,Sigma_all] = derive_ELBO_VI_multi_stimuli_CMP_Matern(signal_rho_len,noise_rho_len_q,xsamp,ysamp,dtbin,tolerance_lambda,use_log,Matern_order)  
    ...
    for j = 1:J
        [ELBO_temp,mu,B_nu,lambda_hat,Sigma] = derive_ELBO_VI_CMP_Matern(squeeze(signal_rho_len(j,:)),noise_rho_len_q,xsamp,squeeze(ysamp(:,:,j)),dtbin,tolerance_lambda,use_log,Matern_order);
        ELBO = ELBO +  ELBO_temp;
        mu_all(:,:,j) = mu;
        Sigma_all(:,:,:,j) = Sigma;
