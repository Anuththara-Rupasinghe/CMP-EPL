%% This function optimizes over the hyper-parameters given observations for multiple stimuli
...
function hyperparameter_final = optimize_hyperparameters_multi_stimuli_CMP_EPL(ysamp,xsamp,dtbin,optimization_method,cordinate_update_iterations_max,min_change_in_hyp,tolerance_lambda)
...
%             options = optimoptions('fmincon','Display','iter','PlotFcn',@optimplotfval,'StepTolerance',1e-2,'OptimalityTolerance',1e-2,'MaxFunctionEvaluations',100);
            ...
            hyperparameter_init = specify_fmincon_par_multi_stimuli_CMP_EPL(J); % Since the domain is constrained in this case, the algorithm converges to the true parameter settings even without a grid search
            ...
            noise_rho_len_q_lb = [hyperparameter_init.noise.rho_lb,hyperparameter_init.noise.len_lb,hyperparameter_init.noise.q_lb]; % Noise hyper-parameter lower bounds
            noise_rho_len_q_ub = [hyperparameter_init.noise.rho_ub,hyperparameter_init.noise.len_ub,hyperparameter_init.noise.q_ub]; % Noise upper-parameter lower bounds
            ...
            if use_log
                ...
                noise_rho_len_q_lb = log(noise_rho_len_q_lb);
                noise_rho_len_q_ub = log(noise_rho_len_q_ub);
            end
    ...
    noise_rho_len_q = [hyperparameter_init.noise.rho_init,hyperparameter_init.noise.len_init,hyperparameter_init.noise.q_init];
            
    if use_log
        signal_rho_len = log(signal_rho_len);
        noise_rho_len_q = log(noise_rho_len_q);
    end
    
    % Store the hyper-parameter updates in each iteration of cordinate descend
    noise_rho_len_q_all = zeros(cordinate_update_iterations_max,3);
    signal_rho_len_all = zeros(cordinate_update_iterations_max,2,J);
    noise_rho_len_q_all(1,:) = noise_rho_len_q;
    ...
    while (iterations_hyp <= cordinate_update_iterations_max)&&(change_in_hyp > min_change_in_hyp)
        ...
        % Estimate the signal hyper-parameters for each stimuli, conditioned on the noise hyper-parameters
        ...
        for j = 1:J
            ...
            estimate_signal = @(signal_rho_len)derive_ELBO_VI_CMP_EPL(signal_rho_len,noise_rho_len_q,xsamp,squeeze(ysamp(:,:,j)),dtbin,tolerance_lambda,use_log);  

            % Update signal hyper-parameters of stimulus p
            switch optimization_method
                case 'fmincon'
        ...
        % Estimate the noise hyper-parameters conditioned on all of the signal hyper-parameters
        fprintf('\nEstimating noise hyper-parameters:\n----------------------------------------------------\n');
        
        % Define the function for updating the noise hyper-parameters
        estimate_noise = @(noise_rho_len_q)derive_ELBO_VI_multi_stimuli_CMP_EPL(signal_rho_len,noise_rho_len_q,xsamp,ysamp,dtbin,tolerance_lambda,use_log);  

        % Update noise hyper-parameters
        switch optimization_method
            case 'fmincon'
                [noise_rho_len_q,~,~,~] = fmincon(estimate_noise,noise_rho_len_q,[],[],[],[],noise_rho_len_q_lb, noise_rho_len_q_ub, [],options);
            case 'fminunc'
                [noise_rho_len_q,~,~,~] = fminunc(estimate_noise,noise_rho_len_q,options);
            case 'fminsearch'
                if iterations_hyp > 2
                    options = optimset('Display','iter','PlotFcns',@optimplotfval,'MaxIter',100,'TolX',1e-2,'TolFun',1e-1,'MaxFunEvals',100); 
                end
                [noise_rho_len_q,~,~,~] = fminsearch(estimate_noise,noise_rho_len_q,options);
        end
        
        % Store the updated hyper-parameters
        noise_rho_len_q_all(1+iterations_hyp,:) = noise_rho_len_q;
        signal_rho_len_all(1+iterations_hyp,:,:) = signal_rho_len';
        
        % Derive the relative change in hyper-paramters to check for convergence
        change_in_hyp_noise = min(abs(noise_rho_len_q_all(1+iterations_hyp,:) - noise_rho_len_q_all(iterations_hyp,:))./abs(noise_rho_len_q_all(1+iterations_hyp,:)));
    ...
    if use_log
        ...
        noise_rho_len_q = exp(noise_rho_len_q);
    end
    ...
    hyperparameter_final.noise.rho = noise_rho_len_q(1);
    hyperparameter_final.noise.len = noise_rho_len_q(2);
    hyperparameter_final.noise.q = noise_rho_len_q(3);
