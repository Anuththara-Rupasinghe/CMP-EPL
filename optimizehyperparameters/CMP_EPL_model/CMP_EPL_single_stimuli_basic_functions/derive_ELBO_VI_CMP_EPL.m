%% This function computes the ELBO given hyper-parameters
...
function [ELBO,mu_all,B_nu,lambda_hat_all,Sigma_all] = derive_ELBO_VI_CMP_EPL(signal_rho_len,noise_rho_len_q,xsamp,ysamp,dtbin,tolerance_lambda,use_log)  
    ...
    noise_rho = (noise_rho_len_q(1));
    noise_len = (noise_rho_len_q(2));
    noise_q = (noise_rho_len_q(3));
    ...
    K_matrix_noise = noise_rho*exp(-0.5*abs((xsamp(:)-xsamp(:)')/noise_len).^noise_q);
