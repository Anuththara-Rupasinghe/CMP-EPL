%% This function optimizes over the hyper-parameters given observations
...
function hyperparameter_final = optimize_hyperparameters_CMP_EPL(ysamp,xsamp,dtbin,optimization_method,cordinate_update_iterations_max,min_change_in_hyp,tolerance_lambda)
...
%             options = optimoptions('fmincon','Display','iter','PlotFcn',@optimplotfval,'StepTolerance',1e-2,'OptimalityTolerance',1e-2,'MaxFunctionEvaluations',100);
        ...
        case 'fminunc'
            ...
            hyperparameter_init = grid_search(xsamp,ysamp,dtbin); % use a grid search to find suitable intialization
    ...
    signal_rho_len_all = zeros(cordinate_update_iterations_max,2);
    ...
    while (iterations_hyp <= cordinate_update_iterations_max)&&(change_in_hyp > min_change_in_hyp)
        ...
        estimate_signal = @(signal_rho_len)derive_ELBO_VI_CMP_EPL(signal_rho_len,noise_rho_len_q,xsamp,ysamp,dtbin,tolerance_lambda,use_log);  
        ...
        estimate_noise = @(noise_rho_len_q)derive_ELBO_VI_CMP_EPL(signal_rho_len,noise_rho_len_q,xsamp,ysamp,dtbin,tolerance_lambda,use_log);  
        ...
        change_in_hyp_signal = min(abs(signal_rho_len_all(1+iterations_hyp,:) - signal_rho_len_all(iterations_hyp,:))./abs(signal_rho_len_all(1+iterations_hyp,:)));
