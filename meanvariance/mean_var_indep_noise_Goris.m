...
function [mean_Goris,variance_Goris,FF_Goris] = mean_var_indep_noise_Goris(r,s_vec,dtbin,bin_size_range)



%% Initialize the variables

mean_Goris = zeros(length(bin_size_range),1);
variance_Goris = zeros(length(bin_size_range),1);
FF_Goris = zeros(length(bin_size_range),1);
...
for j = 1:length(bin_size_range)
    ...
    mean_Goris(j) = mean(mean_temp); % average mean across all bins
    variance_Goris(j) = mean(variance_temp); % average variance across all bins
    FF_Goris(j) = mean(variance_temp./mean_temp); % average Fano Factor across all bins
